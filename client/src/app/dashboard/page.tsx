...
export default function Dashboard() {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      queryClient.setQueryData(["games"], (old: BacklogGame[]) =>
  ...
  const handleStatusChange = async (game: BacklogGame, newStatus: number) => {
    const oldGames = queryClient.getQueryData<BacklogGame[]>(["games"]);
    const updatedGame = { ...game, status: newStatus };
    queryClient.setQueryData(["games"], (old: BacklogGame[]) =>
      old.map((g) => (g.id === game.id ? { ...g, status: newStatus } : g))
    );
        ...
        <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4 border-b border-slate-800 pb-6">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">
              Bem-vindo, {user?.firstName}. Você tem{" "}
              <span className="text-purple-400 font-bold">{games.length}</span>{" "}
              jogos registrados.
            </p>
          </div>
  ...
  return (
    <Card className="bg-slate-900 border-slate-800 overflow-hidden group flex flex-col h-full hover:border-purple-500/50 transition-all shadow-md relative">
      <GameDetailsDialog game={game}>
        <div className="cursor-pointer h-full flex flex-col">
          <div className="relative aspect-[3/4] bg-slate-950 overflow-hidden">
            ...
            <Badge
              variant="outline"
              className={`absolute top-2 right-2 backdrop-blur-md shadow-sm border ${statusConfig.class}`}
            >
              {statusConfig.text}
            </Badge>
          ...
          <div className="p-3 flex-1 flex flex-col gap-2">
            <div className="flex justify-between items-start gap-2">
              <h3
                className="font-bold text-white text-sm leading-tight line-clamp-2 group-hover:text-purple-400 transition-colors"
                title={game.title}
              >
                {game.title}
              </h3>
      ...
      <div className="px-3 pb-3 mt-auto flex items-center justify-between border-t border-slate-800 pt-3">
        <div className="flex gap-1">
          ...
          {game.status !== 2 && (
            <Button
              size="icon"
              variant="ghost"
              className="h-7 w-7 text-slate-500 hover:text-green-400 hover:bg-green-500/10"
              onClick={(e) => {
                e.stopPropagation();
                onStatusChange(2);
              }}
        ...
        <div
          className="flex gap-1 border-l border-slate-800 pl-1 ml-1"
