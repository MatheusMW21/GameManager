...
import { BookOpen, Gamepad2, Loader2, Pencil, Star, Trash2 } from "lucide-react";
...
export default function DiaryPage() {
    ...
    if (half) {
      return (
        <span className="relative inline-block" style={{ width: size, height: size }}>
          <Star size={size} className="absolute inset-0 text-slate-600" strokeWidth={1.8} />
          <span className="absolute inset-y-0 left-0 overflow-hidden" style={{ width: "50%" }}>
            <Star size={size} className="text-cyan-300 fill-cyan-300" strokeWidth={1.8} />
          </span>
        </span>
      );
    }
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const data = await gameService.getDiary();
        setEntries(data || []);
      } catch {
  ...
  const handleUpdate = async () => {
    ...
    setUpdating(true);
    try {
      await gameService.updateReview(editingEntry.gameBacklogItemId ?? 0, editingEntry.id, {
        rating: editRating,
      ...
      setEntries((prev) =>
        prev.map((r) =>
          r.id === editingEntry.id
            ? { ...r, rating: editRating, body: editBody, playedAt: editPlayedAt || null }
            : r
        )
      );
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await gameService.deleteReview(editingEntry.gameBacklogItemId ?? 0, editingEntry.id);
      setEntries((prev) => prev.filter((r) => r.id !== editingEntry.id));
      setEditingEntry(null);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans pb-20">
      ...
      <main className="container mx-auto px-4 py-8 max-w-3xl space-y-6">
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <div>
            ...
            <p className="text-slate-400 text-sm">Suas reviews em ordem cronológica.</p>
          </div>
                      ...
                      <div className="flex-1 p-4 min-w-0">
                        ...
                        <div className="flex items-center gap-3 mb-2">
                          <span className="text-cyan-300 text-xs font-bold flex items-center gap-1">
                            <Star size={11} className="fill-cyan-300" /> {r.rating}/5
                          </span>
      ...
      {/* Edit modal */}
      <Dialog open={!!editingEntry} onOpenChange={(open) => { if (!open) { setEditingEntry(null); setEditHoverRating(null); } }}>
        <DialogContent className="bg-slate-900 border-slate-700 text-slate-100 sm:max-w-[560px] p-0 overflow-hidden">
          <div className="flex gap-5 p-6">
            ...
            <div className="flex-1 space-y-4">
              ...
              <div className="space-y-1">
                <label className="text-xs text-slate-400 font-medium">Jogado em</label>
                <Input
                  type="date"
                  value={editPlayedAt}
              ...
              <div className="space-y-1">
                <label className="text-xs text-slate-400 font-medium">Review</label>
                <Textarea
                  value={editBody}
                ...
                >
                  {[1, 2, 3, 4, 5].map((star, index) => (
                    <button
                      ...
                      onMouseMove={(e) => setEditHoverRating(resolveHalfStep(e, star))}
          ...
          <div className="flex items-center justify-between px-6 py-4 border-t border-slate-800 bg-slate-950/50">
            ...
            <div className="flex gap-2">
              ...
              >
                {updating && <Loader2 size={14} className="animate-spin mr-1" />}
                Salvar
              </Button>
            </div>
          </div>
