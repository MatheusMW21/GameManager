      ...
      <main className="container mx-auto px-4 py-8 flex flex-col items-center min-h-[80vh]">
        ...
        >
          <div className="relative">
            <Search
              className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${isLoading ? "text-purple-500" : "text-slate-500"}`}
            />
            <Input
              value={query}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 w-full max-w-7xl animate-in fade-in slide-in-from-bottom-4 duration-500">
            {results.map((game) => {
              ...
              return (
                <div
                  key={game.id}
                  className="group relative bg-slate-900 border border-slate-800 rounded-xl overflow-hidden hover:border-purple-500/50 hover:shadow-lg hover:shadow-purple-900/20 transition-all flex flex-col"
                >
                  <Link
                  ...
                  <div className="p-4 flex flex-col flex-1 gap-3">
                    ...
                    >
                      <h3
                        className="font-bold text-white text-sm leading-tight line-clamp-2"
                        title={game.name}
                      >
                        {game.name}
                      </h3>
        ...
        {!isLoading &&
          ...
          query.trim() !== "" && (
            <div className="text-center text-slate-500 mt-12 animate-in zoom-in-95 duration-300">
              ...
              <p className="text-sm">
                Não encontramos jogos com o nome "{query}".
              </p>
            </div>
          )}
